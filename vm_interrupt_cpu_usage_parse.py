...
IRQ_CSV = "vm_interrupts_cpu_usage_034517.csv"
...
def datacrunch():
                ...
                if date:
                    ...
                    if (ps_rec_started and (ps_end in line)):
                        ...
                        for irq in IRQ_WATCH_LIST:
                            irq_cpu_usage_list.append(irq_dict[irq])
                        date = ""
                        irq_list.append(irq_cpu_usage_list)
                        irq_cpu_usage_list = []
                        irq_dict = {}               
		            ...
		            cpu_usage = irq_line[10]
                            irq_dict[irq] = cpu_usage
                             
                            

                    if ps_start in line:
                        ps_rec_started = True
                        irq_cpu_usage_list = [date]
    ...
    with open(IRQ_CSV, 'w') as f:
        for irq_cpu_usage_list in irq_list:
            f.write("%s\n" % ','.join(
                                      [ 
                                        str(i) 
                                      ] +
